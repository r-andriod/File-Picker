...
public class GestureImageView extends ImageView {
     ...
     *
     * @see #getGestureMode() ()
     * @see #GESTURE_MODE_SCROLL
     * @see #GESTURE_MODE_SCALE
     * @see #GESTURE_MODE_FREE
     */
    private int mGestureMode = GESTURE_MODE_FREE;
    ...
    public boolean canScrollHorizontally(int direction) {
        if (mGestureMode == GestureImageView.GESTURE_MODE_SCALE) {
            return true;
        }
    ...
    public void outerMatrixTo(Matrix endMatrix, long duration) {
        ...
        mGestureMode = GESTURE_MODE_FREE;
        //停止所有正在进行的动画
        cancelAllAnimator();
        //如果时间不合法立即执行结果
        if (duration <= 0) {
    ...
    public interface OuterMatrixChangedListener {
         ...
         */
        void onOuterMatrixChanged(GestureImageView pinchImageView);
    }
    ...
    private GestureDetector mGestureDetector = new GestureDetector(GestureImageView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (mGestureMode == GESTURE_MODE_FREE && !(mScaleAnimator != null && mScaleAnimator.isRunning())) {
        ...
        public boolean onDoubleTap(MotionEvent e) {
            ...
            if (mGestureMode == GESTURE_MODE_SCROLL && !(mScaleAnimator != null && mScaleAnimator.isRunning())) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (mGestureMode == GESTURE_MODE_SCALE) {
                scaleEnd();
            }
            mGestureMode = GESTURE_MODE_FREE;
            ...
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                ...
                cancelAllAnimator();
                //切换到滚动模式
                mGestureMode = GESTURE_MODE_SCROLL;
                //保存触发点用于move计算差值
            ...
            cancelAllAnimator();
            //切换到缩放模式
            mGestureMode = GESTURE_MODE_SCALE;
            //保存缩放的两个手指
            saveScaleContext(event.getX(0), event.getY(0), event.getX(1), event.getY(1));
        } else if (action == MotionEvent.ACTION_MOVE) {
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                //在滚动模式下移动
                if (mGestureMode == GESTURE_MODE_SCROLL) {
                    //每次移动产生一个差值累积到图片位置上
                    scrollBy(event.getX() - mLastMovePoint.x, event.getY() - mLastMovePoint.y);
                    //记录新的移动点
                    mLastMovePoint.set(event.getX(), event.getY());
                    //在缩放模式下移动
                } else if (mGestureMode == GESTURE_MODE_SCALE && event.getPointerCount() > 1) {
                    //两个缩放点间的距离
